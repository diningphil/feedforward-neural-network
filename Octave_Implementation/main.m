...
## Model assessment ##
#Xtrain = X(1:floor(85*m/100), :);
#Ytrain = Y(1:floor(85*m/100), :);
#Xtest = X(floor(85*m/100 + 1):end, :);
#Ytest = Y(floor(85*m/100 + 1):end, :);

#logDir = startLogging(SIMPLE_T);
#[TR_Err, TR_Acc, nIter, Wres, W_O_res, output_hidden_units, TS_Err_best, TS_Acc_best] = trainWithValidation(Xtrain, Ytrain, Xtest, Ytest, eta, lambda, alpha, outputActivation, hiddenActivation, lossType, estimateMeasure, threshold, init_range, maxIter, layers_dims, runs, useAnnealing, useFanIn, usePlot);
#myPlotWithTest(TR_Err, TR_Acc, nIter, TS_Err_best, TS_Acc_best, lossType, alpha, eta, lambda, layers_dims, size(Y,2), logDir, testName);
#lossFunction = @(O,Y)( leastMeanSquare(O,Y) );
#[err, acc, out] = modelAssessment(Wres, W_O_res, Xtest, Ytest, chooseOutput(outputActivation), chooseHidden(hiddenActivation), lossFunction, estimateMeasure);
#err
#acc
#out
#[TR_Err, TR_Acc, nIter, Wres, W_O_res, output_hidden_units] = train(X, Y, eta, lambda, alpha, outputActivation, hiddenActivation, lossType, estimateMeasure, threshold, init_range, maxIter, layers_dims, runs, useAnnealing, useFanIn, usePlot);
#stopLogging();
#save("./final_weights", "Wres", "W_O_res");
#######################################
