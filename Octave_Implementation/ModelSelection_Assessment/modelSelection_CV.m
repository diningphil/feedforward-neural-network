function [W, W_O, eta_val, alpha_val, lambda_val] = modelSelection_CV(k, X, Y, eta_range, alpha_range, lambda_range, outputActivationType, hiddenActivationType, lossType, estimateFunction, threshold, init_range, maxIter, layers_dims, runs, useAnnealing, useFanIn, showPlots, logging = true)
  logConfig;
  ...
  for e_h = eta_range
    for a_h = alpha_range
      for l_h = lambda_range
          ...
          printf("Mean and standard deviation for the model, computed across different folds\n");
          ...
          % Show results
          #printf("TR error for eta = %f, alpha = %f,lambda = %f for k-fold \n Cost: %f \t Accuracy: %f \n", e_h, a_h, l_h, mean_err_estimate, _acc_estimate);
  ...
  alpha_val
  lambda_val
  printf("Model accuracy variance:\n");
  std_val
  minErr
  minEst
